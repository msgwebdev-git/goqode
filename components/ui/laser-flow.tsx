...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    (uniforms.uWispDensity as { value: number }).value = wispDensity;
    (uniforms.uTiltScale as { value: number }).value = mouseTiltStrength;
    (uniforms.uBeamXFrac as { value: number }).value = horizontalBeamOffset;
    (uniforms.uBeamYFrac as { value: number }).value = verticalBeamOffset;
    (uniforms.uFlowSpeed as { value: number }).value = flowSpeed;
    (uniforms.uVLenFactor as { value: number }).value = verticalSizing;
    (uniforms.uHLenFactor as { value: number }).value = horizontalSizing;
    (uniforms.uFogIntensity as { value: number }).value = fogIntensity;
    (uniforms.uFogScale as { value: number }).value = fogScale;
    (uniforms.uWSpeed as { value: number }).value = wispSpeed;
    (uniforms.uWIntensity as { value: number }).value = wispIntensity;
    (uniforms.uFlowStrength as { value: number }).value = flowStrength;
    (uniforms.uDecay as { value: number }).value = decay;
    (uniforms.uFalloffStart as { value: number }).value = falloffStart;
    (uniforms.uFogFallSpeed as { value: number }).value = fogFallSpeed;

    const { r, g, b } = hexToRGB(color || "#FFFFFF");
    (uniforms.uColor as { value: THREE.Vector3 }).value.set(r, g, b);
  }, [
    wispDensity,
    mouseTiltStrength,
