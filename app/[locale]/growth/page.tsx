...
export async function generateMetadata({
  ...
  return {
    ...
    keywords: t("growth.keywords"),
    openGraph: {
      title: t("growth.ogTitle"),
      description: t("growth.ogDescription"),
      type: "website",
      url: await getLocalizedUrl("/growth", locale as Locale),
      siteName: "GoQode",
      locale: getOgLocale(locale as Locale),
    },
    alternates: await getAlternates("/growth", locale as Locale),
  };
}
...
async function BreadcrumbJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: t("growth"), item: await getLocalizedUrl("/growth", locale as Locale) },
    ],
  };
  return (
    <script
