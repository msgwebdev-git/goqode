...
export async function generateMetadata({
  ...
  return {
    ...
    keywords: t("support.keywords"),
    openGraph: {
      title: t("support.ogTitle"),
      description: t("support.ogDescription"),
      type: "website",
      url: await getLocalizedUrl("/support", locale as Locale),
      siteName: "GoQode",
      locale: getOgLocale(locale as Locale),
    },
    alternates: await getAlternates("/support", locale as Locale),
  };
}
...
async function BreadcrumbJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: t("support"), item: await getLocalizedUrl("/support", locale as Locale) },
    ],
  };
  return (
    <script
