...
export async function generateMetadata({
  ...
  return {
    ...
    keywords: t("platforms.keywords"),
    openGraph: {
      title: t("platforms.ogTitle"),
      description: t("platforms.ogDescription"),
      type: "website",
      url: await getLocalizedUrl("/platforms", locale as Locale),
      siteName: "GoQode",
      locale: getOgLocale(locale as Locale),
    },
    alternates: await getAlternates("/platforms", locale as Locale),
  };
}
...
async function BreadcrumbJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: t("platforms"), item: await getLocalizedUrl("/platforms", locale as Locale) },
    ],
  };
  return (
    <script
