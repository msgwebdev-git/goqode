...
const SERVICE_HREF = "/services/corporate-website";
...
async function ServiceJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    serviceType: t("corporateWebsite"),
    areaServed: ["RO", "MD", "EU"],
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name: t("corporateWebsitePackages"),
      itemListElement: [
        {
          "@type": "Offer",
          priceCurrency: "EUR",
...
async function BreadcrumbJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: t("corporateWebsite"), item: serviceUrl },
    ],
  };
  return (
    <script
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      ...
      <FaqJsonLd locale={locale} namespace="ServiceCorporate" />
      <CorporateContent />
    </>
  );
}
