...
const SERVICE_HREF = "/services/web-application-development";
...
async function ServiceJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    serviceType: t("webApp"),
    areaServed: ["RO", "MD", "EU"],
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name: t("webAppPackages"),
      itemListElement: [
        {
          "@type": "Offer",
          priceCurrency: "EUR",
...
async function BreadcrumbJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: t("webApp"), item: serviceUrl },
    ],
  };
  return (
    <script
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      ...
      <FaqJsonLd locale={locale} namespace="ServiceWebapp" />
      <WebAppContent />
    </>
  );
}
