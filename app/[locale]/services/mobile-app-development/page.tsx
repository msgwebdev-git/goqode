...
const SERVICE_HREF = "/services/mobile-app-development";
...
async function ServiceJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    serviceType: t("mobileApp"),
    areaServed: ["RO", "MD", "EU"],
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name: t("mobileAppPackages"),
      itemListElement: [
        {
          "@type": "Offer",
          priceCurrency: "EUR",
...
async function BreadcrumbJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: t("mobileApp"), item: serviceUrl },
    ],
  };
  return (
    <script
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      ...
      <FaqJsonLd locale={locale} namespace="ServiceMobile" />
      <MobileContent />
    </>
  );
}
