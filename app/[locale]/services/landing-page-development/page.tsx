...
const SERVICE_HREF = "/services/landing-page-development";
...
async function ServiceJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    serviceType: t("landingPage"),
    areaServed: ["RO", "MD", "EU"],
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name: t("landingPagePackages"),
      itemListElement: [
        {
          "@type": "Offer",
          priceCurrency: "EUR",
...
async function BreadcrumbJsonLd({ locale }: { locale: string }) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: t("landingPage"), item: serviceUrl },
    ],
  };
  return (
    <script
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      ...
      <FaqJsonLd locale={locale} namespace="ServiceLanding" />
      <LandingPageContent />
    </>
  );
}
